class FlightsController < ApplicationController
  ...
  def index
  	...
  	@welcome_msg ="Hello! Welcome to Flight Mileage Tracker!"
...
# !!!!!!!!!!!!!!!!!!!!!!! remember to add it back!!!!
  ...
  def destroy
    ...
    ff_record.destroy
    redirect_to '/flights'  # go back to index where rails pull all db record again from scratch
  end

  private
  def ff_params
  	params.require(:ff).permit(:airline, :ff_no, :points, :user_id ,:password, :description)
  end
  ...
  def american(ff_no, ff_password)  
    ...
    # if (agent.page.at('.account-info'))
      ...
      if info.nil? # means password/ username incorrect
        ...
        mileage = info.children[9].text.split.join('')
...
# test it on my own site , codes here and the ones i tried successfully on rails c directly talking to the airine site
      ...
      agent.get('https://www.southwest.com/flight/login')
...
# there are two id for recently login user and the rest
  ...
  def lufthansa(ff_no, ff_password)  
  # 2 things to do:
  #  first -> update this mileage to db (ff_id is needed here)
  # second -> print all data again to index again (so ff_id is no longer needed)
  # render text: ff_id
    agent = Mechanize.new
    agent.get('https://www.lufthansa.com/online/portal/lh/us/profile_login?l=en&cid=1000390')
    form = agent.page.form_with(:name=>"usernameForm")
    form.PC_7_CO19VHUC6NNB00AIG06KNQ2U22012478_userId = ff_no  
    # nprincess322
    form.PC_7_CO19VHUC6NNB00AIG06KNQ2U22012478_password = ff_password
    # wifi123
    form.submit


    # info = agent.page.at('#user-btn')
    info = agent.page.at('#contentpage')
    div = info.xpath('//div')
    div.at('div.pi_groupContainerContent')
    # but do not have    ---- div.at('profile-block') 
    


    
    # div.at('div.cl')
    # it should be in there.... but need to dig it out...

    # not working
    # div = info.xpath("//div")bv
    # info.div_with(:id=>'#social-messages')
    

    div[0]
    # div = info.xpath("//div//profile-block//cl")
    bv



    if info.nil? # means password/ usernameprofile-block//cl incorrect
      mileage = "Unmatch reward no. or password!"
    else
    # render text: info
    # info[4]
